"""Exception handlers, carried forward from module 07 unchanged in behaviour."""
...
def build_response(
    ...
    payload = ErrorResponse(
        error=ErrorBody(code=code, message=message, details=details or [], reference=get_request_id())
    )
...
async def handle_unexpected_error(request: Request, exc: Exception) -> JSONResponse:
    ...
    if settings.debug and not settings.is_production:
        # The exception TYPE only. str(exc) is never included, not even in
        # debug: exception messages routinely carry connection strings, SQL
        # fragments and file paths. A contract test in module 08 caught this
        # leaking a full postgres:// URL, which is why the message is gone.
        details = [
            ErrorDetail(
                field=None,
