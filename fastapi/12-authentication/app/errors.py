...
class UnauthenticatedError(AppError):
    """No credentials, or credentials that cannot be trusted.

    401 means "I do not know who you are". The WWW-Authenticate header is part
    of the spec for this status and tells the client how to authenticate; the
    handler adds it.
    """
...
class ForbiddenError(AppError):
    ...
    Returning 401 here would tell a client to log in again, which cannot help
    and produces an endless re-authentication loop.
    """
...
class UpstreamTimeoutError(UpstreamError):
    """A dependency was reachable but too slow.

    504 rather than 503, because the distinction is useful to a client: a
    service that is down may come back in seconds, whereas one that is merely
    slow is likely to be slow again on an immediate retry.
    """
